...
class triangleList : public simpleModel
{
public:
	triangleList(vector<glm::vec3> _vertices);
};
...
{
public:
	solidCone(const float angle, const int slices, const glm::vec3 direction, const float height);
	solidCone(const int slices);
	~solidCone();
	void upload();
	void draw();
protected:
...
{
public:
	solidCylinder(const int slices);
	~solidCylinder();
	void upload();
	void draw();
protected:
